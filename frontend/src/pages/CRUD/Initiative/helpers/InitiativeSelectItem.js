...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/initiative/autocomplete`, { params });
  return response.data;
}

const InitiativeSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.initiative.hasPermissionToCreate,
});

export default connect(select)(InitiativeSelectItem);
