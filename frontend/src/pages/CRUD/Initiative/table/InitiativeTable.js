...
import * as dataFormat from 'pages/CRUD/Initiative/table/InitiativeDataFormatters';

import actions from 'actions/initiative/initiativeListActions';
...
const InitiativeTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Initiative Details', title: 'Initiative_Details' },
  ]);
  ...
  const count = useSelector((store) => store.initiative.list.count);
  const modalOpen = useSelector((store) => store.initiative.list.modalOpen);
  const rows = useSelector((store) => store.initiative.list.rows);
  const idToDelete = useSelector((store) => store.initiative.list.idToDelete);
  ...
  const getInitiativeCSV = async () => {
    const response = await axios({
      url: '/initiative?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'initiativeCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'Initiative_Details',

      flex: 0.6,

      headerName: 'Initiative Details',
    },
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Initiative')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/initiative/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/initiative/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Initiative'
                : 'http://localhost:8080/api-docs/#/Initiative'
            }
          >
            API documentation for initiative
          </LinkMaterial>
        </div>
      </Widget>
