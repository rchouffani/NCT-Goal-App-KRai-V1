...
import * as dataFormat from 'pages/CRUD/Opco_address/table/Opco_addressDataFormatters';

// eslint-disable-next-line
import * as opcoDataFormat from 'pages/CRUD/Opco/table/OpcoDataFormatters';
// eslint-disable-next-line
import * as address_stateDataFormat from 'pages/CRUD/Address_state/table/Address_stateDataFormatters';

import actions from 'actions/opco_address/opco_addressListActions';
...
const Opco_addressTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Latit', title: 'latit' },
    { label: 'Address Name', title: 'Address_Name' },

    { label: 'Op Co', title: 'OpCo' },
    { label: 'Address State', title: 'address_state' },
  ]);
  ...
  const count = useSelector((store) => store.opco_address.list.count);
  const modalOpen = useSelector((store) => store.opco_address.list.modalOpen);
  const rows = useSelector((store) => store.opco_address.list.rows);
  const idToDelete = useSelector((store) => store.opco_address.list.idToDelete);
  ...
  const getOpco_addressCSV = async () => {
    const response = await axios({
      url: '/opco_address?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'opco_addressCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        opcoDataFormat.listFormatter(params.row[params.field], history, 'opco'),
      flex: 1,
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        address_stateDataFormat.listFormatter(
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Opco_address')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/opco_address/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/opco_address/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Opco_address'
                : 'http://localhost:8080/api-docs/#/Opco_address'
            }
          >
            API documentation for opco_address
          </LinkMaterial>
        </div>
      </Widget>
...
export default Opco_addressTable;
