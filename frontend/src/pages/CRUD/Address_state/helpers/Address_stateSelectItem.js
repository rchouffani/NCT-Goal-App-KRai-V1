...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/address_state/autocomplete`, { params });
  return response.data;
}

const Address_stateSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.address_state.hasPermissionToCreate,
});

export default connect(select)(Address_stateSelectItem);
