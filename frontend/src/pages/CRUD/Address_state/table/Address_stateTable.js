...
import * as dataFormat from 'pages/CRUD/Address_state/table/Address_stateDataFormatters';

import actions from 'actions/address_state/address_stateListActions';
...
const Address_stateTable = () => {
  ...
  const count = useSelector((store) => store.address_state.list.count);
  const modalOpen = useSelector((store) => store.address_state.list.modalOpen);
  const rows = useSelector((store) => store.address_state.list.rows);
  const idToDelete = useSelector(
    (store) => store.address_state.list.idToDelete,
  );
  ...
  const getAddress_stateCSV = async () => {
    const response = await axios({
      url: '/address_state?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'address_stateCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Address_state')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/address_state/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/address_state/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Address_state'
                : 'http://localhost:8080/api-docs/#/Address_state'
            }
          >
            API documentation for address_state
          </LinkMaterial>
        </div>
      </Widget>
...
export default Address_stateTable;
