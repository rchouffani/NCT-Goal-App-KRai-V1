...
import * as dataFormat from 'pages/CRUD/Journey/table/JourneyDataFormatters';

// eslint-disable-next-line
import * as opcoDataFormat from 'pages/CRUD/Opco/table/OpcoDataFormatters';

import actions from 'actions/journey/journeyListActions';
...
const JourneyTable = () => {
  ...
  const count = useSelector((store) => store.journey.list.count);
  const modalOpen = useSelector((store) => store.journey.list.modalOpen);
  const rows = useSelector((store) => store.journey.list.rows);
  const idToDelete = useSelector((store) => store.journey.list.idToDelete);
  ...
  const getJourneyCSV = async () => {
    const response = await axios({
      url: '/journey?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Journey')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/journey/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/journey/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Journey'
                : 'http://localhost:8080/api-docs/#/Journey'
            }
          >
            API documentation for journey
          </LinkMaterial>
        </div>
      </Widget>
