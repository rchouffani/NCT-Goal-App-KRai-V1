...
import * as dataFormat from 'pages/CRUD/Goal_type/table/Goal_typeDataFormatters';

import actions from 'actions/goal_type/goal_typeListActions';
...
const Goal_typeTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    { label: 'Goal Type', title: 'Goal_Type' },
    { label: 'Goal Type Details', title: 'Goal_Type_Details' },
  ]);
  ...
  const count = useSelector((store) => store.goal_type.list.count);
  const modalOpen = useSelector((store) => store.goal_type.list.modalOpen);
  const rows = useSelector((store) => store.goal_type.list.rows);
  const idToDelete = useSelector((store) => store.goal_type.list.idToDelete);
  ...
  const getGoal_typeCSV = async () => {
    const response = await axios({
      url: '/goal_type?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'goal_typeCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'Goal_Type_Details',

      flex: 0.6,

      headerName: 'Goal Type Details',
    },
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Goal_type')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/goal_type/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/goal_type/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Goal_type'
                : 'http://localhost:8080/api-docs/#/Goal_type'
            }
          >
            API documentation for goal_type
          </LinkMaterial>
        </div>
      </Widget>
