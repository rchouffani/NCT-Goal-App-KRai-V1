...
const CustomerTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    { label: 'Customer Name', title: 'Customer_Name' },
    { label: 'Customer Id Other', title: 'Customer_id_Other' },
    { label: 'Customer Id CW', title: 'Customer_id_CW', number: 'true' },
  ...
  const getCustomerCSV = async () => {
    const response = await axios({
      url: '/customer?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  const columns = [
    ...
    {
      field: 'Customer_id_CW',

      flex: 0.6,

      headerName: 'Customer Id CW',
    },

    {
      field: 'Customer_id_Other',

      flex: 0.6,

      headerName: 'Customer Id Other',
    },
    ...
    <div>
      <Widget title={<h4>{humanize('Customer')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/customer/new' className={classes.element}>
