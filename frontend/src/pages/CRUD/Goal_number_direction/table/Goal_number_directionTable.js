...
import * as dataFormat from 'pages/CRUD/Goal_number_direction/table/Goal_number_directionDataFormatters';

import actions from 'actions/goal_number_direction/goal_number_directionListActions';
...
const Goal_number_directionTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    { label: 'Goal Number Direction', title: 'Goal_Number_Direction' },
  ]);
  ...
  const count = useSelector((store) => store.goal_number_direction.list.count);
  const modalOpen = useSelector(
    (store) => store.goal_number_direction.list.modalOpen,
  );
  const rows = useSelector((store) => store.goal_number_direction.list.rows);
  const idToDelete = useSelector(
    (store) => store.goal_number_direction.list.idToDelete,
  );
  ...
  const getGoal_number_directionCSV = async () => {
    const response = await axios({
      url: '/goal_number_direction?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'goal_number_directionCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'Goal_Number_Direction',

      flex: 0.6,

      headerName: 'Goal Number Direction',
    },
  ...
  return (
    <div>
      <Widget
        title={<h4>{humanize('Goal_number_direction')}</h4>}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/goal_number_direction/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getGoal_number_directionCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/goal_number_direction/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Goal_number_direction'
                : 'http://localhost:8080/api-docs/#/Goal_number_direction'
            }
          >
            API documentation for goal_number_direction
          </LinkMaterial>
        </div>
      </Widget>
...
export default Goal_number_directionTable;
