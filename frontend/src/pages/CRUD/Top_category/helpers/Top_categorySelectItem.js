...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/top_category/autocomplete`, { params });
  return response.data;
}

const Top_categorySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.top_category.hasPermissionToCreate,
});

export default connect(select)(Top_categorySelectItem);
