...
import * as dataFormat from 'pages/CRUD/Top_category/table/Top_categoryDataFormatters';

// eslint-disable-next-line
import * as initiativeDataFormat from 'pages/CRUD/Initiative/table/InitiativeDataFormatters';

import actions from 'actions/top_category/top_categoryListActions';
...
const Top_categoryTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    { label: 'Top Category', title: 'Top_Category' },
    { label: 'Top Category Details', title: 'Top_Category_Details' },
  ...
  const count = useSelector((store) => store.top_category.list.count);
  const modalOpen = useSelector((store) => store.top_category.list.modalOpen);
  const rows = useSelector((store) => store.top_category.list.rows);
  const idToDelete = useSelector((store) => store.top_category.list.idToDelete);
  ...
  const getTop_categoryCSV = async () => {
    const response = await axios({
      url: '/top_category?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'top_categoryCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'Top_Category_Details',

      flex: 0.6,

      headerName: 'Top Category Details',
    },

    {
      ...
      sortable: false,
      renderCell: (params) =>
        initiativeDataFormat.listFormatter(
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Top_category')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/top_category/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/top_category/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Top_category'
                : 'http://localhost:8080/api-docs/#/Top_category'
            }
          >
            API documentation for top_category
          </LinkMaterial>
        </div>
      </Widget>
...
export default Top_categoryTable;
