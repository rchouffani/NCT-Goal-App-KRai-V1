...
// eslint-disable-next-line
import * as top_categoryDataFormat from 'pages/CRUD/Top_category/table/Top_categoryDataFormatters';
...
const CategoryTable = () => {
  ...
  const columns = [
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        top_categoryDataFormat.listFormatter(
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Category')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/category/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/category/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
