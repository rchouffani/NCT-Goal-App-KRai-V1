...
import * as dataFormat from 'pages/CRUD/Opco_contact/table/Opco_contactDataFormatters';
...
import actions from 'actions/opco_contact/opco_contactListActions';
...
const Opco_contactTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Full Name', title: 'Full_Name' },
  ...
  const count = useSelector((store) => store.opco_contact.list.count);
  const modalOpen = useSelector((store) => store.opco_contact.list.modalOpen);
  const rows = useSelector((store) => store.opco_contact.list.rows);
  const idToDelete = useSelector((store) => store.opco_contact.list.idToDelete);
  ...
  const getOpco_contactCSV = async () => {
    const response = await axios({
      url: '/opco_contact?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'opco_contactCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Opco_contact')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/opco_contact/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/opco_contact/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Opco_contact'
                : 'http://localhost:8080/api-docs/#/Opco_contact'
            }
          >
            API documentation for opco_contact
          </LinkMaterial>
        </div>
      </Widget>
...
export default Opco_contactTable;
