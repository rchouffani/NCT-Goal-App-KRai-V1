...
import * as dataFormat from 'pages/CRUD/Opco/table/OpcoDataFormatters';

import actions from 'actions/opco/opcoListActions';
...
const OpcoTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    { label: 'Op Co Name', title: 'OpCo_Name' },
  ]);
  ...
  const count = useSelector((store) => store.opco.list.count);
  const modalOpen = useSelector((store) => store.opco.list.modalOpen);
  const rows = useSelector((store) => store.opco.list.rows);
  const idToDelete = useSelector((store) => store.opco.list.idToDelete);
  ...
  const getOpcoCSV = async () => {
    const response = await axios({
      url: '/opco?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Opco')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/opco/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/opco/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Opco'
                : 'http://localhost:8080/api-docs/#/Opco'
            }
          >
            API documentation for opco
          </LinkMaterial>
        </div>
      </Widget>
